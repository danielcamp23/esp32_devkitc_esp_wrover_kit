                             ...
                             CK_OBJECT_HANDLE_PTR xObjectHandle )
{
    ...
    if( xResult == CKR_OK )
    {
        ...
        {
            lConversionReturn = key_prov_pem_to_der( xCertificateTemplate.xValue.pValue,
 ...
 *
 */
int key_prov_pem_to_der( const unsigned char * pucInput,
                        size_t xLen,
...
CK_RV key_prov_device( CK_SESSION_HANDLE xSession,
                        ProvisioningParams_t * pxParams )
{
    ...
    xResult = key_prov_certificate( xSession,
    ...
    if( CKR_OK == xResult )
    {
        ...
        if( pucDerObject != NULL )
        {
            lConversionReturn = key_prov_pem_to_der( xPrivateKeyTemplate.xValue.pValue,
...
void key_prov_alternate_load_keys( ProvisioningParams_t * xParams )
{
    ...
    xResult = key_prov_initialize_session( &xFunctionList,
                                           &xSlotId,
                                           &xSession );

    if( xResult == CKR_OK )
    {
        xResult = key_prov_device( xSession, xParams );
    }
...
void key_prov_load_keys( void )
{
    ...
    key_prov_alternate_load_keys( &xParams );
}